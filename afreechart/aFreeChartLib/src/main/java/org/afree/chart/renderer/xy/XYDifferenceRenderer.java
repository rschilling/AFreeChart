 ...
 * <code>DifferenceChartDemo1.java</code> program included in the AFreeChart
 * demo collection:
 * <br><br>
...
public class XYDifferenceRenderer extends AbstractXYItemRenderer
    ...
    static {
        DEFAULT_POSITIVE_PAINT.setColor(Color.GREEN);
        DEFAULT_NEGATIVE_PAINT.setColor(Color.RED);
    }
    ...
    public XYDifferenceRenderer() {
        this(new Paint(DEFAULT_POSITIVE_PAINT), 
                new Paint(DEFAULT_NEGATIVE_PAINT), 
                false);
    }
    ...
    public XYItemRendererState initialise(Canvas canvas,
        ...
        XYItemRendererState state = super.initialise(canvas, dataArea, plot, data,
                info);
    ...
    public void drawItem(Canvas canvas,
        ...
        if (pass == 0) {
            drawItemPass0(canvas, dataArea, info, plot, domainAxis, rangeAxis,
                    dataset, series, item, crosshairState);
        }
        else if (pass == 1) {
            drawItemPass1(canvas, dataArea, info, plot, domainAxis, rangeAxis,
                    dataset, series, item, crosshairState);
        }
     ...
     *                          (<code>null</code> permitted).
     */
    protected void drawItemPass0(Canvas x_graphics,
                                 RectShape x_dataArea,
     ...
     *                          (<code>null</code> permitted).
     */
    protected void drawItemPass1(Canvas x_graphics,
                                 RectShape x_dataArea,
                                 PlotRenderingInfo x_info,
                                 XYPlot x_plot,
                                 ValueAxis x_domainAxis,
        ...
        Float l_seriesStroke = getItemStroke(x_series, x_item);
        PathEffect l_seriesEffect = getItemEffect(x_series, x_item);
        ...
        if (getShapesVisible()) {
            ...
            if (l_shape.intersects(x_dataArea)) {
                Paint paint = PaintUtility.createPaint(
                        ...
                        getItemPaintType(x_series, x_item));
                l_shape.fill(x_graphics, paint);
            }
        ...
        if (null != l_entities) {
            if (null == l_entityArea) {
                l_entityArea = new RectShape((l_x1 - 2), (l_y1 - 2),
                        4, 4);
            }
            String l_tip = null;
            ...
            String l_url = null;
            // TODO: implement getURLGenerator
...
//            }
        ...
        LineShape l_line = null;
        if (PlotOrientation.HORIZONTAL == l_orientation) {
            l_line = new LineShape(l_y1, l_x1, l_y2, l_x2);
        }
        else if (PlotOrientation.VERTICAL == l_orientation) {
            l_line = new LineShape(l_x1, l_y1, l_x2, l_y2);
        }
     ...
     *                   of type Double).
     */
    private void createPolygon (Canvas x_graphics,
                                RectShape x_dataArea,
                                XYPlot x_plot,
                                ValueAxis x_domainAxis,
                                ValueAxis x_rangeAxis,
        ...
        PathShape l_path = new PathShape();
        ...
        if (l_path.intersects(x_dataArea)) {
            Paint paint;
            if ( x_positive ) {
                paint = new Paint(getPositivePaint());
            } else {
                paint = new Paint(getNegativePaint());
            }
            
            l_path.fill(x_graphics, paint);
        }
    }
        ...
        if (p != null) {
            ...
            if (dataset != null) {
                if (getItemVisible(series, 0)) {
                    ...
                    // TODO: implement getLegendItemToolTipGenerator
...
//                    }
                    ...
                    float stroke = lookupSeriesStroke(series);
                    
                    Shape line = getLegendLine();
                    result = new LegendItem(label, description,
                            toolTipText, urlText, line, stroke, paintType);
    ...
    public boolean equals(Object obj) {
        ...
        if (!this.positivePaint.equals(that.positivePaint)) {
            return false;
        }
        if (!this.negativePaint.equals(that.negativePaint)) {
            return false;
        }
