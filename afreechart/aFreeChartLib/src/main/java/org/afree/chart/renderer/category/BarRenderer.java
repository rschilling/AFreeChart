 ...
 * program included in the AFreeChart Demo Collection: <br>
 * <br>
...
public class BarRenderer extends AbstractCategoryItemRenderer implements
    ...
    public BarRenderer() {
        ...
        this.gradientPaintTransformer = new StandardGradientShaderFactory();
        this.minimumBarLength = 0.0;
        setBaseLegendShape(new RectShape(-4.0, -4.0, 8.0, 8.0));
        this.barPainter = getDefaultBarPainter();
        this.shadowsVisible = getDefaultShadowsVisible();
        this.shadowPaintType = new SolidColor(Color.GRAY);
    ...
    public CategoryItemRendererState initialise(Canvas canvas,
        ...
        CategoryItemRendererState state = super.initialise(canvas, dataArea, plot,
                rendererIndex, info);
     ...
     *            the renderer state.
     */
    protected void calculateBarWidth(CategoryPlot plot, RectShape dataArea,
    ...
    public LegendItem getLegendItem(int datasetIndex, int series) {
        ...
        LegendItem result = new LegendItem(label, description, toolTipText,
                urlText, true, shape, true, paintType,
                isDrawBarOutline(), outlinePaintType, outlineStroke,
                false, new LineShape(), 1.0f, new SolidColor(Color.BLACK));
    ...
    public void drawItem(Canvas canvas, CategoryItemRendererState state,
        ...
        RectShape bar = null;
        if (orientation == PlotOrientation.HORIZONTAL) {
            bar = new RectShape(barL0 - barL0Adj, barW0, barLength
                    + barLengthAdj, state.getBarWidth());
        } else {
            bar = new RectShape(barW0, barL0 - barL0Adj, state
                    .getBarWidth(), barLength + barLengthAdj);
        }
        if (getShadowsVisible()) {
            this.barPainter.paintBarShadow(canvas, this, row, column, bar, barBase,
                    true);
        }
     ...
     */
    protected void drawItemLabel(Canvas canvas, CategoryDataset data, int row,
            int column, CategoryPlot plot,
            CategoryItemLabelGenerator generator, RectShape bar,
            boolean negative) {
        ...
        PaintType paintType = getItemLabelPaintType(row, column);
        ...
        Paint paint = PaintUtility.createPaint(Paint.ANTI_ALIAS_FLAG, paintType, labelFont);
     ...
     */
    private PointF calculateLabelAnchorPoint(ItemLabelAnchor anchor,
            RectShape bar, PlotOrientation orientation) {
