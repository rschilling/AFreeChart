...
public class CombinedDomainCategoryPlot extends CategoryPlot
    ...
    public void moveRangeAxes(double movePercent, PlotRenderingInfo info, PointF source) {
        CategoryPlot p = findSubplot(info, source);
        
        if(p instanceof Movable) {
            for (int i = 0; i < p.getRangeAxisCount(); i++) {
                ValueAxis rangeAxis = (ValueAxis) p.getRangeAxis(i);
                if (rangeAxis != null) {
    ...
    protected AxisSpace calculateAxisSpace(Canvas canvas,
        ...
        if (fixed != null) {
            ...
            if (categoryAxis != null) {
                space = categoryAxis.reserveSpace(canvas, this, plotArea,
                        categoryEdge, space);
            }
            else {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (orientation == PlotOrientation.HORIZONTAL) {
                ...
                this.subplotAreas[i] = new RectShape(x, y, w,
                        adjustedPlotArea.getHeight());
                x = x + w + this.gap;
            }
            else if (orientation == PlotOrientation.VERTICAL) {
                double h = usableSize * plot.getWeight() / totalWeight;
                this.subplotAreas[i] = new RectShape(x, y,
                        adjustedPlotArea.getWidth(), h);
                y = y + h + this.gap;
            }

            AxisSpace subSpace = plot.calculateRangeAxisSpace(canvas,
    ...
    public void draw(Canvas canvas,
        ...
        AxisState axisState = axis.draw(canvas, cursor, area, dataArea,
                domainEdge, info);
        if (parentState == null) {
        ...
        for (int i = 0; i < this.subplots.size(); i++) {
            ...
            plot.draw(canvas, this.subplotAreas[i], subAnchor, parentState,
                    subplotInfo);
        }
     ...
     * the AFreeChart code will never call this method (although this is not
     * checked/enforced).
      *
      * @param axis  the axis.
      *
