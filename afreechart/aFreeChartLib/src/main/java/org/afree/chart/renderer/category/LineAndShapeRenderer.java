 ...
 * (for use with the {@link CategoryPlot} class). The example shown here is
 * generated by the <code>LineChartDemo1.java</code> program included in the
...
public class LineAndShapeRenderer extends AbstractCategoryItemRenderer
     ...
     * 
     * @deprecated As of JFreeChart 1.0.7 (this override flag is unnecessary).
     */
    ...
    /**
     * Sets the flag that controls whether or not the x-position for each data
     * item is offset within its category according to the series, and sends a
     * {@link RendererChangeEvent} to all registered listeners.
     * 
     * @param offset
     *            the offset.
    ...
    /**
     * Returns the item margin, which is the gap between items within a category
     * (expressed as a percentage of the overall category width). This can be
     * used to match the offset alignment with the bars drawn by a
     * {@link BarRenderer}).
     * 
     ...
     * (expressed as a percentage of the overall category width), and sends a
     * {@link RendererChangeEvent} to all registered listeners.
     * 
     * @param margin
     *            the margin (0.0 <= margin < 1.0).
     * 
     * @see #getItemMargin()
     * @see #getUseSeriesOffset()
     * 
    ...
    public LegendItem getLegendItem(int datasetIndex, int series) {
        ...
        if (isSeriesVisible(series) && isSeriesVisibleInLegend(series)) {
            ...
            PaintType fillPaintType = (this.useFillPaint ? getItemFillPaintType(series, 0)
                    : paintType);
            boolean shapeOutlineVisible = this.drawOutlines;
            PaintType outlinePaintType = (this.useOutlinePaintType ? getItemOutlinePaintType(
                    series, 0) : paintType);
            ...
            LegendItem result = new LegendItem(label, description, "", "",
                    shapeVisible, shape, getItemShapeFilled(series, 0),
                    fillPaintType, shapeOutlineVisible, outlinePaintType, 
                    outlineStroke, lineVisible,
                    new LineShape(-7.0, 0.0, 7.0, 0.0), getItemStroke(
                            series, 0), getItemPaintType(series, 0));
    ...
    public void drawItem(Canvas canvas, CategoryItemRendererState state,
        ...
        if (this.useSeriesOffset) {
            x1 = domainAxis.getCategorySeriesMiddle(column, dataset
                    .getColumnCount(), visibleRow, visibleRowCount,
        ...
        if (pass == 0 && getItemLineVisible(row, column)) {
            if (column != 0) {
                ...
                if (previousValue != null) {
                    ...
                    double x0;
                    if (this.useSeriesOffset) {
                        x0 = domainAxis.getCategorySeriesMiddle(column - 1,
                                dataset.getColumnCount(), visibleRow,
                                visibleRowCount, this.itemMargin, dataArea,
                    ...
                    if (orientation == PlotOrientation.HORIZONTAL) {
                        line = new LineShape(y0, x0, y1, x1);
                    } else if (orientation == PlotOrientation.VERTICAL) {
                        line = new LineShape(x0, y0, x1, y1);
                    }
                    
                    Paint paint = PaintUtility.createPaint(
                            Paint.ANTI_ALIAS_FLAG,
                            getItemPaintType(row, column),
                            getItemStroke(row, column),
                            getItemEffect(row, column));
        ...
        if (pass == 1) {
            ...
            if (getItemShapeVisible(row, column)) {
                if (getItemShapeFilled(row, column)) {
                    ...
                    if (this.useFillPaint) {
                        paintType = getItemFillPaintType(row, column);
                    } else {
                        paintType = getItemPaintType(row, column);
                    }
                ...
                if (this.drawOutlines) {
                    ...
                    if (this.useOutlinePaintType) {
                        paintType = getItemOutlinePaintType(row, column);
                    } else {
                        paintType = getItemPaintType(row, column);
                    }
                    Paint paint = PaintUtility.createPaint(
            ...
            if (isItemLabelVisible(row, column)) {
                if (orientation == PlotOrientation.HORIZONTAL) {
                    drawItemLabel(canvas, orientation, dataset, row, column, y1,
                            x1, (value < 0.0));
                } else if (orientation == PlotOrientation.VERTICAL) {
                    drawItemLabel(canvas, orientation, dataset, row, column, x1,
                            y1, (value < 0.0));
                }
            }
            ...
            updateCrosshairValues(state.getCrosshairState(), dataset
                    .getRowKey(row), dataset.getColumnKey(column), value,
