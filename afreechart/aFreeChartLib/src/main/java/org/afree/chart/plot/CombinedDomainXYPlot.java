...
public class CombinedDomainXYPlot extends XYPlot
     ...
     * only the domain axis is shared between subplots, the AFreeChart code
    ...
    /**
     * Returns the gap between subplots, measured in Canvas units.
     *
    ...
    public void draw(Canvas canvas,
        ...
        for (int i = 0; i < this.subplots.size(); i++) {
            ...
            plot.draw(canvas, this.subplotAreas[i], anchor, parentState,
                    subplotInfo);
        }
    ...
    public void moveRangeAxes(double movePercent, PlotRenderingInfo info, PointF source) {
        XYPlot p = findSubplot(info, source);
